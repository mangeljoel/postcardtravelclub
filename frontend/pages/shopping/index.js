...
import Shopping from "../../src/features/Shopping";

const ShoppingPage = ({ post }) => {
  const getSeoValues = () => {
    let seoData = {
      title: post?.name ? post.name : "Shopping | Conscious Luxury Travel ",
      description: post?.description
        ? post.description
        : "Discover conscious culinary experiences around the world",
      url: "https://www.postcard.travel/shopping",
      image: post?.logo?.url ? post?.logo?.url : "/assets/images/stays.jpg",
      keywords:
        "Postcard Shopping, Conscious Luxury properties,Conscious Luxury hotels",
    };
    return seoData;
  };
  return (
...
export async function getStaticProps({ params, preview = null }) {
  ...
  try {
    data = await strapi.find("directories", {
      filters: { slug: "shopping-conscious-luxury-travel" },
      populate: {
        logo: {
          fields: ["url"],
        },
