...
const GuidedSearchFnD = ({ getFilters, isDiary, filter, setFilter, slug, aff, callBeforeApply }) => {
    ...
    return (
        <Box id={"GuidedSearchAlbum"} w={"100%"} px={["0%", "10%"]} position={"relative"} >
            ...
            {totalFnds && totalFnds.length > 0 && <Flex w={"100%"} h={["9.167vw", "3.472vw"]} justify={"center"}>
                {!(filter?.country || filter?.region || filter?.environment || (filter?.selectedTags?.length > 0)) ? (
                    <Button
                        variant="solid"
                        bg="#EB836E"
                        color="white"
            ...
            <Flex id={"GuidedSearchBody"} flexDirection={"column"} display={open ? "flex" : "none"} maxH={["85vh", "43.96vw"]} h={["auto", "43.96vw"]} mt={["-9.167vw", "-3.472vw"]} borderRadius={["4.5vw", "2.083vw"]} px={["8.33vw", "3.05vw"]} py={["7.77vw", "2.64vw"]} w={["100%", "80%"]} mx={[0, "10%"]} bg={"white"} zIndex={60} position={"absolute"} left={0} justifyContent={"space-between"}>
                ...
                {currentTab == "country" && <FilterDisplay
                    ...
                    isActive={(filter?.country || filter?.region || filter?.environment || (filter?.selectedTags?.length > 0))}
                />}
                ...
                {currentTab == "type" && environments?.length > 0 && <FilterDisplay
                    type={2}
                ...
                {currentTab == "cuisines" && categories?.length > 0 && <FilterDisplay
                    type={3}
...
export default GuidedSearchFnD