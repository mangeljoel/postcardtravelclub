...
const SearchResultCards = ({ searchFilter, slug, isDiary, type, appliedTag, loading, canEdit, openEditForm, setLoading }) => {
            ...
            >
                ...
                >
                    <Text
                        ...
                        textAlign="center"
                    >
                        {isActive ? "You have 0 Restaurants collected, Start Collecting now :)" : "No Restaurants Collected"}
                    </Text>
                    {isActive && (
                        <Text
                            as={Link}
    ...
    return (
        <Box mb="2em" mt={["0", "2em"]}>
            ...
            {profile && !isDiary && filteredData?.length > 6 && ["Admin", "SuperAdmin", "EditorInChief", "EditorialAdmin"].includes(profile?.user_type?.name) && <Button mb="2em" onClick={() => setOpenCreateModal({ show: true, mode: "create" })}>Add New Restaurant</Button>}

            <DxStaysForm
                isOpen={createModal.show}
                headerText={createModal.mode == "create" ? "Create Restaurant" : "Edit Restaurant"}
                onClose={() => {
