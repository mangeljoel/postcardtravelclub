...
const GuidedSearchPostcard = ({ type, filter, setFilter, selectedTag, setSelectedTag, forProfile = false, slug, aff }) => {
        ...
        return (tabName) => {
            ...
            // If modal is closed, use filter state
                ...
                if (
                    ...
                    (tabName === 'tag-group' && filter?.tagGroups) ||
    ...
    }, [open, currentTab, selectedCountry, selectedTagGroup, selectedTags, filter])
    ...
    const handleNextClick = () => {
        if (currentTab === 'country') setCurrentTab('tag-group');
    ...
    const handleApply = () => {
        setFilter({
            ...
            tagGroups: selectedTagGroup?.id !== -1 ? selectedTagGroup : null,
            selectedTags,
        });
        setOpen(false);
    };
    ...
    const handleReset = () => {
        setFilter({ country: null, tagGroups: null, selectedTags: [] });
    ...
    const fetchTagGroups = useCallback(async () => {
        const queryParts = !aff && !slug ? ['type=hotels'] : []; // on affiliation and user profile page postacrds can be from hotels or tours
        if (forProfile && slug) {
            queryParts.push(`profile=${slug}`)
            queryParts.push(`resultFor=postcards`)
        }
        if (aff) queryParts.push(`affiliation=${aff}`)
        if (selectedCountry && selectedCountry?.id !== -1) queryParts.push(`country=${selectedCountry?.id}`);
        const tagGroupQuery = queryParts.length ? `?${queryParts.join('&')}` : '';

        const tagGroups = await strapi.find(`tag-groups/getTagGroups${tagGroupQuery}`);
        setTagGroups(tagGroups);
    }, [selectedCountry])

    const fetchTags = useCallback(async () => {
        const queryParts = !aff && !slug ? ['type=hotels'] : [];
        if (forProfile && slug) {
            queryParts.push(`profile=${slug}`)
            queryParts.push(`resultFor=postcards`)
        }
        ...
        const tags = await strapi.find(`tags/getTags${query}`);
        setTags(tags);
        // setCurrentTab('interest');
    }, [selectedCountry, selectedTagGroup]);
    ...
    useEffect(() => {
        if (open) {
            ...
            setSelectedTagGroup(filter?.tagGroups)
    ...
    //         setSelectedTag(null);
    //         setFilter({ selectedTags: [selectedTag] });
    //         document.getElementById('GuidedSearchPostcard')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    //     }
    // }, [selectedTag]);
    ...
    return (
        <Box id={"GuidedSearchPostcard"} w={"100%"} px={["0%", "10%"]} position={"relative"} >
            ...
            <Flex w={"100%"} h={["9.167vw", "3.472vw"]} justify={"center"}>
                ...
                >
                    {open ?
                        (selectedTagGroup ? selectedTagGroup.name : "Theme") :
                        (filter?.tagGroups ? filter?.tagGroups.name : "Theme")
                    }
                </Flex>
                <Flex
                    {...{
            ...
            <Flex id={"GuidedSearchBody"} flexDirection={"column"} display={open ? "flex" : "none"} maxH={["85vh", "43.96vw"]} h={["auto", "43.96vw"]} mt={["-9.167vw", "-3.472vw"]} borderRadius={["4.5vw", "2.083vw"]} px={["8.33vw", "3.05vw"]} py={["7.77vw", "2.64vw"]} w={["100%", "80%"]} mx={[0, "10%"]} bg={"white"} zIndex={60} position={"absolute"} left={0} justifyContent={"space-between"}>
                    ...
                    tagGroups?.length > 0 ?
                        <Flex
                            ...
                            <Flex
                                ...
                                pb={["15vw", "10vw"]}
                            >
                                <Text as={Flex} px={["8.33vw", "2.67vw"]} minH={["8.33vw", "3.125vw"]} fontSize={["3.33vw", "1.25vw"]} borderRadius={["7.22vw", "2.78vw"]} bg={selectedTagGroup?.id == -1 ? "primary_1" : "#D9D9D9"} color={selectedTagGroup?.id == -1 ? "white" : "#111111"} w={"fit-content"} justifyContent={"center"} alignItems={"center"} fontFamily={"raleway"} fontWeight={500} cursor={"pointer"}
                                    onClick={() => {
                                ...
                                >{selectedTagGroup?.id == -1 ? "Clear All" : "Select All"}
                                </Text>
                                <Flex mt={["4vw", "2.98vw"]} flexWrap={"wrap"} gap={["1.67vw", "0.833vw"]}>
                                    {tagGroups?.map((tagGroup) => (
                                        <Text
                                            ...
                                            borderRadius={["7.22vw", "2.78vw"]}
                                            bg={selectedTagGroup?.id == -1 || selectedTagGroup?.id == tagGroup.id ? "primary_1" : "#D9D9D9"}
                                            color={selectedTagGroup?.id == -1 || selectedTagGroup?.id == tagGroup.id ? "white" : "#111111"}
                                            w={"fit-content"}
                        ...
                        :
                        <Flex h={"90%"} justify={"center"} alignItems={"center"}>
                            <Text fontSize={["3.61vw", "1.875vw"]} fontFamily={"lora"} fontStyle={"italic"}>
                                {"No Themes" + (selectedCountry ? ` in ${selectedCountry?.name}` : "")}
                            </Text>
                        </Flex>
                )
                }
                ...
                {currentTab == "interest" && (
                    tags?.length > 0 ?
                        ...
                        <Flex h={"90%"} justify={"center"} alignItems={"center"}>
                            <Text fontSize={["3.61vw", "1.875vw"]} fontFamily={"lora"} fontStyle={"italic"}>
                                {"No Interests" + (selectedTagGroup ? ` in ${selectedTagGroup?.name}` : selectedCountry ? ` in ${selectedCountry?.name}` : "")}
                            </Text>
                        </Flex>
                )
                }
