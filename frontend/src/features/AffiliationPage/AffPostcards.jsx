...
const AffPostcards = ({ aff }) => {
    ...
    const fetchPostcards = useCallback(async () => {
        ...
        try {
            ...
            const { data } = await axios.post(
                `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/postcards/getPostcards${queryParts.length > 0 ? `?${queryParts.join('&')}` : ''}`,
                {
    ...
    // Fetch postcards whenever page resets to 0 after a filter or slug change
    useEffect(() => {
        if (page === 1 && hasMore) {
            fetchPostcards();
        }
    ...
    >
        ...
        <GuidedSearchPostcard type={"postcards"} filter={filter} setFilter={setFilter} aff={aff} />
